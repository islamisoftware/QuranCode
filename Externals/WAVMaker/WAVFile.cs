...
public class WAVFile
{
    ...
    public byte[] GetNextSample_ByteArray()
    {
        ...
        {
            audioSample = null;
            throw new WAVFileReadException(ex.Message, "WAVFile.GetNextSample_ByteArray()");
        }
    ...
    public void AddSample_ByteArray(byte[] pSample)
    {
        if (pSample != null)
        {
            ...
            {
                throw new WAVFileWriteException(ex.Message, "WAVFile.AddSample_ByteArray()");
            }
        }
    }
    ...
    static public bool IsWaveFile(String pFilename)
    {
        ...
        if (File.Exists(pFilename))
        {
            ...
            {
                throw new WAVFileException(ex.Message, "WAVFile.IsWaveFile()");
            }
        }
    ...
    public void SeekToAudioSample(long pSampleNum)
    {
        if (mFileStream != null)
        {
            ...
            try
            {
                ...
                mNumSamplesRemaining = (int)(NumSamples - pSampleNum); // keep NumSamplesRemaining property up-to-date 
            }
            catch (IOException ex)
            {
                throw new WAVFileIOException("Unable to to seek to sample " + pSampleNum.ToString() + ": " + ex.Message,
            ...
            {
                throw new WAVFileException(ex.Message, "WAVFile.SeekToAudioSample()");
            }
        }
    }
    ...
    private static void Normalize(ref List<long> values, long minimum, long maximum)
    {
        ...
        for (int i = 0; i < values.Count; i++)
        {
            if (min_value > values[i]) min_value = values[i];
            if (max_value < values[i]) max_value = values[i];
        }

        if ((max_value - min_value) > 0)
        {
            double factor = (maximum - minimum) / (max_value - min_value);
            for (int i = 0; i < values.Count; i++)
            {
