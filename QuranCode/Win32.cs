...
public class Win32
{
    ...
    /// <summary>
    /// Get handle to the system tray
    /// </summary>
    /// <returns></returns>
    public static IntPtr GetTrayHandle()
    {
        IntPtr tray_handle = FindWindow("Shell_TrayWnd", null);
        if (!tray_handle.Equals(IntPtr.Zero))
        {
            return FindWindowEx(tray_handle, IntPtr.Zero, "TrayNotifyWnd", IntPtr.Zero);
        }
        return IntPtr.Zero;
    }
    
    /// <summary>
    /// get the RECT of the system tray
    /// </summary>
    /// <returns></returns>
    public static Rectangle GetTrayRectangle()
    {
        RECT tray_rectangle = new RECT();
        GetWindowRect(GetTrayHandle(), out tray_rectangle);
        return new Rectangle(new Point(tray_rectangle.left, tray_rectangle.top), new Size((tray_rectangle.right - tray_rectangle.left) + 1, (tray_rectangle.bottom - tray_rectangle.top) + 1));
    }
